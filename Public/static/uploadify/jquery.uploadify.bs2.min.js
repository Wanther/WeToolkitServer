...
swfUploadSettings=$.extend(swfUploadSettings,settings);var playerVersion=swfobject.getFlashPlayerVersion();var flashInstalled=(playerVersion.major>=9);if(flashInstalled){window['uploadify_'+settings.id]=new SWFUpload(swfUploadSettings);var swfuploadify=window['uploadify_'+settings.id];$this.data('uploadify',swfuploadify);var $wrapper=$('<div />',{'id':settings.id});$('#'+swfuploadify.movieName).wrap($wrapper);$wrapper=$('#'+settings.id);$wrapper.data('uploadify',swfuploadify);var $button=$('<div />',{'id':settings.id+'-button','class':settings.buttonClass});if(settings.buttonImage){$button.css({'background-image':"url('"+settings.buttonImage+"')",'text-indent':'-9999px'});}
$button.html(settings.buttonText);$wrapper.append($button);$('#'+swfuploadify.movieName).css({'position':'absolute','z-index':1});if(!settings.queueID){var $queue=$('<div />',{'id':settings.id+'-queue','class':'uploadify-queue'});$wrapper.after($queue);swfuploadify.settings.queueID=settings.id+'-queue';swfuploadify.settings.defaultQueue=true;}
swfuploadify.queueData={files:{},filesSelected:0,filesQueued:0,filesReplaced:0,filesCancelled:0,filesErrored:0,uploadsSuccessful:0,uploadsErrored:0,averageSpeed:0,queueLength:0,queueSize:0,uploadSize:0,queueBytesUploaded:0,uploadQueue:[],errorMsg:'Some files were not added to the queue:'};swfuploadify.original=$clone;swfuploadify.wrapper=$wrapper;swfuploadify.button=$button;swfuploadify.queue=$queue;if(settings.onInit)settings.onInit.call($this,swfuploadify);}else{if(settings.onFallback)settings.onFallback.call($this);}});},cancel:function(fileID,supressEvent){var args=arguments;this.each(function(){var $this=$(this),swfuploadify=$this.data('uploadify'),settings=swfuploadify.settings,delay=-1;if(args[0]){if(args[0]=='*'){var queueItemCount=swfuploadify.queueData.queueLength;$('#'+settings.queueID).find('.uploadify-queue-item').each(function(){delay++;if(args[1]===true){swfuploadify.cancelUpload($(this).attr('id'),false);}else{swfuploadify.cancelUpload($(this).attr('id'));}
$(this).find('.data').removeClass('data').html(' - 取消');$(this).find('.uploadify-progress-bar').remove();$(this).delay(1000+100*delay).fadeOut(500,function(){$(this).remove();});});swfuploadify.queueData.queueSize=0;swfuploadify.queueData.queueLength=0;if(settings.onClearQueue)settings.onClearQueue.call($this,queueItemCount);}else{for(var n=0;n<args.length;n++){swfuploadify.cancelUpload(args[n]);$('#'+args[n]).find('.data').removeClass('data').html(' - 取消');$('#'+args[n]).find('.uploadify-progress-bar').remove();$('#'+args[n]).delay(1000+100*n).fadeOut(500,function(){$(this).remove();});}}}else{var item=$('#'+settings.queueID).find('.uploadify-queue-item').get(0);$item=$(item);swfuploadify.cancelUpload($item.attr('id'));$item.find('.data').removeClass('data').html(' - 取消');$item.find('.uploadify-progress-bar').remove();$item.delay(1000).fadeOut(500,function(){$(this).remove();});}});},destroy:function(){this.each(function(){var $this=$(this),swfuploadify=$this.data('uploadify'),settings=swfuploadify.settings;swfuploadify.destroy();if(settings.defaultQueue){$('#'+settings.queueID).remove();}
...
itemData={'fileID':file.id,'instanceID':settings.id,'fileName':fileName,'fileSize':fileSize}
if(settings.itemTemplate==false){settings.itemTemplate='<div id="${fileID}" class="uploadify-queue-item alert alert-success">\
...
if($.inArray('onSelect',settings.overrideEvents)<0){itemHTML=settings.itemTemplate;for(var d in itemData){itemHTML=itemHTML.replace(new RegExp('\\$\\{'+d+'\\}','g'),itemData[d]);}
...
if(settings.onUploadComplete)settings.onUploadComplete.call(this,file);},onUploadError:function(file,errorCode,errorMsg){var settings=this.settings;var errorString='Error';switch(errorCode){case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:errorString='服务器拒绝 ('+errorMsg+')';break;case SWFUpload.UPLOAD_ERROR.MISSING_UPLOAD_URL:errorString='无效的上传地址';break;case SWFUpload.UPLOAD_ERROR.IO_ERROR:errorString='读取文件失败';break;case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:errorString='不安全的文件';break;case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:alert('文件大小超过 ('+errorMsg+').');errorString='文件大小超过限制';break;case SWFUpload.UPLOAD_ERROR.UPLOAD_FAILED:errorString='失败';break;case SWFUpload.UPLOAD_ERROR.SPECIFIED_FILE_ID_NOT_FOUND:break;case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:errorString='校验失败';break;case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:errorString='取消';this.queueData.queueSize-=file.size;this.queueData.queueLength-=1;if(file.status==SWFUpload.FILE_STATUS.IN_PROGRESS||$.inArray(file.id,this.queueData.uploadQueue)>=0){this.queueData.uploadSize-=file.size;}
if(settings.onCancel)settings.onCancel.call(this,file);delete this.queueData.files[file.id];break;case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED:errorString='停止';break;}
if($.inArray('onUploadError',settings.overrideEvents)<0){if(errorCode!=SWFUpload.UPLOAD_ERROR.FILE_CANCELLED&&errorCode!=SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED){$('#'+file.id).removeClass('alert-success').addClass('uploadify-error').addClass('alert-danger');}
$('#'+file.id).find('.uploadify-progress-bar').css('width','1px');if(errorCode!=SWFUpload.UPLOAD_ERROR.SPECIFIED_FILE_ID_NOT_FOUND&&file.status!=SWFUpload.FILE_STATUS.COMPLETE){$('#'+file.id).find('.data').html(' - '+errorString);}}
...
if($.inArray('onUploadProgress',settings.overrideEvents)<0){if(settings.progressData=='percentage'){$('#'+file.id).find('.data').html(' - '+percentage+'%');}else if(settings.progressData=='speed'&&lapsedTime>500){$('#'+file.id).find('.data').html(' - '+this.queueData.averageSpeed+suffix);}
...
if(settings.checkExisting){$.ajax({type:'POST',async:false,url:settings.checkExisting,data:{filename:file.name},success:function(data){if(data==1){var overwrite=confirm('文件 "'+file.name+'" 已存在.\n覆盖已存在的文件?');if(!overwrite){this.cancelUpload(file.id);$('#'+file.id).remove();if(this.queueData.uploadQueue.length>0&&this.queueData.queueLength>0){if(this.queueData.uploadQueue[0]=='*'){this.startUpload();}else{this.startUpload(this.queueData.uploadQueue.shift());}}}}}});}
if(settings.onUploadStart)settings.onUploadStart.call(this,file);},onUploadSuccess:function(file,data,response){var settings=this.settings;var stats=this.getStats();this.queueData.uploadsSuccessful=stats.successful_uploads;this.queueData.queueBytesUploaded+=file.size;if($.inArray('onUploadSuccess',settings.overrideEvents)<0){$('#'+file.id).find('.data').html(' - 完成');}
